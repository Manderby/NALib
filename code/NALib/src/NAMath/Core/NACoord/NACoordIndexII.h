...
NA_IDEF size_t naGetSizei64IndexCount(NASizei64 size) {
    ...
    if(!naIsSizei64Useful(size))
      naError("size is not useful.");
  #endif
  return naCasti64ToSize_t(naMuli64(size.width, size.height));
}
...
NA_IDEF size_t naGetVolumei64IndexCount(NAVolumei64 volume) {
    ...
    if(!naIsVolumei64Useful(volume))
      naError("volume is not useful.");
  #endif
  return naCasti64ToSize_t(naMuli64(naMuli64(volume.width, volume.height), volume.depth));
}
...
NA_IDEF size_t naGetIndexWithOrigini64AndPosRowFirst(NAPosi64 origin, NAPosi64 offset, int64 width) {
  return naCasti64ToSize_t(naAddi64(naMuli64(naSubi64(offset.y, origin.y), width), naSubi64(offset.x, origin.x)));
}
...
NA_IDEF size_t naGetIndexWithOrigini64AndPosColumnFirst(NAPosi64 origin, NAPosi64 offset, int64 height) {
  return naCasti64ToSize_t(naAddi64(naMuli64(naSubi64(offset.x, origin.x), height), naSubi64(offset.y, origin.y)));
}
...
NA_IDEF size_t naGetIndexWithOrigini64AndVertexRowFirst(NAVertexi64 origin, NAVertexi64 vertex, int64 width, int64 height) {
  return naCasti64ToSize_t(naAddi64(naMuli64(naAddi64(naMuli64(naSubi64(vertex.z, origin.z), height), naSubi64(vertex.y, origin.y)), width), naSubi64(vertex.x, origin.x)));
}
...
NA_IDEF size_t naGetIndexWithOrigini64AndVertexColumnFirst(NAVertexi64 origin, NAVertexi64 vertex, int64 depth, int64 height) {
  return naCasti64ToSize_t(naAddi64(naMuli64(naAddi64(naMuli64(naSubi64(vertex.x, origin.x), height), naSubi64(vertex.y, origin.y)), depth), naSubi64(vertex.z, origin.z)));
}
