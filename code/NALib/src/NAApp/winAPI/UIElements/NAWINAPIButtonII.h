...
NA_DEF NAButton* naNewTextPushButton(const NAUTF8Char* text, double width) {
  ...
  HWND nativePtr = CreateWindow(
    ...
    0,
    0,
    (LONG)naRound(winapiButton->rect.size.width * uiScale),
    (LONG)naRound(winapiButton->rect.size.height * uiScale),
...
NA_HDEF void na_SetButtonRect(NA_UIElement* button, NARect rect) {
  ...
  SetWindowPos(
    ...
    HWND_TOP,
    (LONG)naRound(winapiButton->rect.pos.x * uiScale),
    (LONG)naRound((parentRect.size.height - winapiButton->rect.pos.y - winapiButton->rect.size.height) * uiScale),
    (LONG)naRound(winapiButton->rect.size.width * uiScale),
    (LONG)naRound(winapiButton->rect.size.height * uiScale),
    0);
}
