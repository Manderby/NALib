...
NA_DEF NASpace* naNewSpace(NASize size) {
  ...
  double uiScale = naGetUIElementResolutionScale(NA_NULL);
    ...
    0,
    0,
    (LONG)(winapiSpace->rect.size.width * uiScale),
    (LONG)(winapiSpace->rect.size.height * uiScale),
...
NA_HDEF void na_SetSpaceRect(NA_UIElement* space, NARect rect) {
  ...
  SetWindowPos(
    ...
    HWND_TOP,
    (LONG)(winapiSpace->rect.pos.x * uiScale),
    (LONG)((parentRect.size.height - winapiSpace->rect.pos.y - winapiSpace->rect.size.height) * uiScale),
    (LONG)(winapiSpace->rect.size.width * uiScale),
    (LONG)(winapiSpace->rect.size.height * uiScale),
    SWP_NOZORDER);
...
NA_HDEF void na_ForceSpaceToRedrawBackground(NA_UIElement* space) {
  NAWINAPISpace* winapiSpace = (NAWINAPISpace*)space;
  winapiSpace->forceEraseBackground = NA_TRUE;
  naRefreshUIElement(space, 0);
